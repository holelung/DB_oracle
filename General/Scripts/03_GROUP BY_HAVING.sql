...
SELECT NVL(DEPT_CODE, '없음') 부서명, SUM(SALARY) 합계
FROM EMPLOYEE 
GROUP BY DEPT_CODE ;
...
/**
 * GROUP BY를 통해서 만들어진 그룹 중 조건에 맞는 그룹만 선택
 */
...
-- 부서 코드가 'D5'인 사원의 이름, 부서 코드, 직급 코드 조회
SELECT
