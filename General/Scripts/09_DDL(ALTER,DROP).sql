...
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_TITLE_UN UNIQUE(DEPT_TITLE);
...
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_ID_CHK CHECK (LOCATION_ID IN ('L1','L2','L3','L4','L5'));
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NULL; -- NULL 허용 O
...
ALTER TABLE DEPT_COPY 
DROP CONSTRAINT DEPT_COPY_TITLE_UN;

ALTER TABLE DEPT_COPY 
DROP CONSTRAINT DEPT_COPY_ID_CHK;
...
--> 기존에 저장된 데이터의 값보다 작은 크기의 자료형으로 변경 불가
...
--> 이미 '한국'이라고 값이 채워져 있어서 있는 값이 바뀌지는 않음
...
COMMIT; -- TRANSACTION 변경사항 데이터베이스에 반영
...
--> 테이블은 행과 열을 가지는 DB객체로 
-- 	최소한 한 개 이상의 컬럼을 가져야 한다!
