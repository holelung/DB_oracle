...
FROM EMPLOYEE
WHERE EMAIL LIKE '____%'; -- 전체조회됨
--> "EMAIL이 4글자 이상이면 조회" 라는 의미로 해석

/* 발생한 문제 : 
 * "구분자"로 사용하려던 '_'가 LIKE의 와일드카드 '_'로 해석되면서 문제발생
 * 
 * [해결 방법]
 * - LIKE ESCAPE OPTION 이용
 * 	 -> 지정된 특수문자 뒤 '딱 한 글자' 를
 * 			와일드카드가 아닌 단순 문자열로 인식 시키는 옵션
 * 
 * - 작성법
