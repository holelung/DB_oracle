...
WHERE 
	LENGTH(EMAIL) <= 12
ORDER BY 
	LENGTH(EMAIL) ASC; -- 16행 조회

-----------------------------------------

/* INSTR(문자열 | 컬럼명, '찾을 문자열' [, 시작위치 [, 순번]]) 
 * - 시작 위치부터 지정된 순번 까지
 *   문자열 | 컬럼값 에서 '찾을 문자열'의 위치를 반환
 * */
...
-- 오늘 날짜 변환
-- '2025년 02월 20일 (목) 오후 04시 30분 20초' 
SELECT
	TO_CHAR(SYSDATE,
...
-- RPAD(문자열, 길이, 바꿀문자)
--> 문자열 전체에서 오른쪽을 지정된 길이 만큼 바꿀문자로 변경
...
SELECT
	...
	EMP_NO,
	DECODE(
		SUBSTR(EMP_NO,8,1), 1, '남자', 2, '여자') AS 성별
FROM
	EMPLOYEE;
...
FROM 
	EMPLOYEE 
WHERE 
	DEPT_CODE = 'D6'; -- 4900000	3800000
	
	
-- 모든 사원 중 이름순으로 오름 차순 정렬 했을 때
