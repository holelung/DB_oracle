...
SELECT E.EMP_ID, E.EMP_NAME, J.JOB_NAME, D.DEPT_ID, E.SALARY, E.HIRE_DATE 
FROM EMPLOYEE E 
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
LEFT OUTER JOIN DEPARTMENT D ON(D.DEPT_ID = E.DEPT_CODE)
WHERE 
...
SELECT
	E.EMP_ID, E.EMP_NAME, J.JOB_NAME, D.DEPT_TITLE, E.SALARY 
FROM EMPLOYEE E
...
HAVING COUNT(*) >= 3;
-- 그룹함수를 제외하고 나머지 컬럼은 GROUP BY와 동일해야한다?

-- 부서별 인원 수가 가장 적은 부서의
-- 부서명, 인원 수 조회
-- 단, 부서 이름이 없으면 '없음'으로 표시
SELECT 
	NVL(D.DEPT_TITLE, '없음') AS DETP_TITLE, 
	COUNT(*)
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY D.DEPT_TITLE  
...
SELECT E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE, '없음'), J.JOB_NAME
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE);
...
SELECT 
	...
	CASE 
		-- 현재 행의 EMP_ID와 
		-- 일치하는 값이 서브쿼리 결과에 있으면 '사수', 없으면 '사원'
		WHEN E.EMP_ID IN (
...
FROM EMPLOYEE 
WHERE DEPT_CODE IN ( -- 2) 서브쿼리
	SELECT DEPT_ID
	FROM DEPARTMENT 
	WHERE LOCATION_ID = ( -- 1) 서브쿼리
		SELECT LOCAL_CODE
		FROM LOCATION 
		WHERE NATIONAL_CODE ='KO'	
	)
...
SELECT
	E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, E.JOB_CODE, D.DEPT_TITLE, J.JOB_NAME
FROM EMPLOYEE E
...
SELECT 
	EMP_ID, EMP_NAME, DEPT_CODE, MANAGER_ID, EMP_NO, HIRE_DATE 
FROM EMPLOYEE 
...
/* EXISTS(서브쿼리)
 * - 서브쿼리 조회 결과가 1행 이상 존재하면 TRUE, 0행이면 FALSE
 * */
...
WHERE 
	...
	/* 상호연관 쿼리 해석순서
	 * 1) 메인 쿼리 한줄 해석
	 * 2) 해석된 메인쿼리 1행의 컬럼값을 서브쿼리에 사용
	 * 
	 * 3) (메인 쿼리 WHEREw절 사용시)
	 * 		서브쿼리의 결과 먼저 해석된 메인 쿼리 1행에 영향을 줌
	 * 	-> 최정결과(RESULT SET)에 포함될 지 안될지를 개
	 * */
...
WHERE 
	...
	FROM EMPLOYEE SUB 
	WHERE 
		NVL(SUB.DEPT_CODE, '소속없음') = NVL(MAIN.DEPT_CODE, '소속없음')
...
-- 2) 각 직원들이 속한 직급의 급여 평균 조회
SELECT
	MAIN.EMP_NAME, MAIN.JOB_CODE,
	(
		SELECT AVG(SALARY)
		FROM EMPLOYEE SUB
...
SELECT 
	MAIN.EMP_ID, MAIN.EMP_NAME,
	NVL(MAIN.MANAGER_ID, '없음') AS "관리자 사번",
	NVL((
		SELECT  SUB.EMP_NAME
		FROM EMPLOYEE SUB
...
-- 4) 급여 내림차순으로 정렬 후 ROWNUM <= 5이하만 조회
SELECT
...
--> SELECT 해석 순서 문제로 정상 결과X

--> [해결 방법] : INLINE-VIEW 사용
...
WHERE (DEPT_CODE, JOB_CODE) = (
	SELECT E2.DEPT_CODE, E2.JOB_CODE
	FROM EMPLOYEE E2
	WHERE EXTRACT (YEAR FROM HIRE_DATE) = 2010
);
...
SELECT 
	MAIN.EMP_ID, MAIN.EMP_NAME,
	NVL(D.DEPT_TITLE,'소속없음'), J.JOB_NAME,
	MAIN.HIRE_DATE
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT D ON (D.DEPT_ID = MAIN.DEPT_CODE)
...
SELECT 
	MAIN.EMP_ID, MAIN.EMP_NAME,
	J.JOB_NAME,
	FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('19'||SUBSTR(MAIN.EMP_NO,0,6),'YYYYMMDD'))/12) AS "만 나이",
	TO_CHAR(SALARY*(1+NVL(BONUS,0))*12,'L999,999,999') AS "연봉"
FROM EMPLOYEE MAIN
JOIN JOB J ON (J.JOB_CODE = MAIN.JOB_CODE)
WHERE SUBSTR(MAIN.EMP_NO,0,6) = (
	SELECT 
		MAX(TO_NUMBER(SUBSTR(SUB.EMP_NO,0,6)))
	FROM
		EMPLOYEE SUB
	WHERE 
		SUB.JOB_CODE = MAIN.JOB_CODE 
