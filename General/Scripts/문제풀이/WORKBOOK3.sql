...
HAVING AVG(G.POINT) = (
	...
	FROM(
		SELECT AVG(G.POINT) AS AVG_GRADE
		FROM TB_STUDENT S
		JOIN TB_DEPARTMENT D ON (D.DEPARTMENT_NO = S.DEPARTMENT_NO)
		JOIN TB_GRADE G ON (G.STUDENT_NO = S.STUDENT_NO)
		WHERE D.DEPARTMENT_NAME ='국어국문학과'
...
-- inline
-- 국어국문학과 학생들 평점 조회 해서 ROWNUM으로 조회하는 방법으로 해보기
...
SELECT 
	...
	D.CATEGORY 
FROM TB_DEPARTMENT D 
JOIN TB_DEPARTMENT D2 ON (D2.CATEGORY = D.CATEGORY)
