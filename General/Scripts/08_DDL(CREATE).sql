 ...
 * 이름					: MEMBER_NAME VARCHAR2(15) 	(한글 5글자)
 * 주민등록 번호	: MEMBER_SSN  CHAR(14) 			(- 포함)
 * 가입일				: EMROLL_DATE	DATE
 * 	-> 기본값을 SYSDATE로 지정
 */
...
COMMENT ON COLUMN "MEMBER".MEMBER_SSM IS '회원 주민등록번호';
COMMENT ON COLUMN "MEMBER".ENROLL_DATE IS '회원 가입일';

ALTER TABLE "MEMBER" RENAME COLUMN MEMBER_SSM TO MEMBER_SSN;
...
-- 가입일 -> DEFAULT 활용(테이블 생성시 정의된 값이 반영)
INSERT INTO "MEMBER"
VALUES('MEM04', '123453123ABC', '박철수', '970226-1243543', DEFAULT);
--> ENROLL_DATE 컬럼에 설정된 DEFAULT값 삽

-- 가입일 --> INSERT시 미작성 하는 경우 -> DEFAULT값이 반영됨
INSERT INTO "MEMBER" (MEMBER_ID, MEMBER_PW, MEMBER_NAME, MEMBER_SSN)
VALUES('MEM05', '1q2w3e4r', '신짱구', '000202-3243543');


-- SSM오타 여서 수정함
DELETE FROM "MEMBER" WHERE MEMBER_ID = 'MEM05';
...
--    USER_NO NUMBER NOT NULL, -- 제약조건 미지정 시 "SYS-" 형태로 자동 지정
...
CREATE TABLE USER_USED_UK(
    ...
    -- 테이블레벨로 USER_ID 컬럼에 UNIQUE 제약조건 지정
...
-- 한 테이블당 한 개만 설정할 수 있음
	--> PK 제약조건이 1개
	--> PK가 설정된 컬럼은 1개 또는 여러 개 (PK 복합 키)
...
CREATE TABLE USER_USED_FK(
  ...
  /* 두 테이블의 똑같은 값이 작성되어 데이터 중복으로 보이지만
...
-- 무결성 제약조건(KH19_JJH.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK2(
  ...
  -- 컬럼 레벨 설정
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK2 REFERENCES USER_GRADE2(GRADE_CODE)ON DELETE SET NULL
);
...
CREATE TABLE USER_USED_FK3(
  ...
  -- 테이블 레벨
--  CONSTRAINT GRADE_CODE_FK3 -- 제약 조건명 설정(생략가능)
  FOREIGN KEY (GRADE_CODE) REFERENCES USER_GRADE3(GRADE_CODE) ON DELETE CASCADE
...
SELECT * FROM USER_USED_FK3; -- 10을 참조하던 홍길동, 이순신 삭제됨
...
SELECT *
FROM USER_USED_FK CHILD
LEFT JOIN USER_GRADE PARENT ON (CHILD.GRADE_CODE  = PARENT.GRADE_CODE);
----------------------------------------------------------------------------------------------------------------
 ...
 *  조건을 수정하는데 어려움이 있다
 * 
 * -> 조건이 다양한 경우에는 FK제약조건을 이용하는 것이 좋다.
 * */
...
CREATE TABLE CHECK_TEST(
	...
	TEST_SCORE NUMBER
	CONSTRAINT CHECK_SOCORE CHECK(TEST_SCORE BETWEEN 1 AND 100)
);
INSERT INTO CHECK_TEST
VALUES('KIM', 100);

INSERT INTO CHECK_TEST VALUES('SON', 93);

INSERT INTO CHECK_TEST VALUES('DONY', 0);
-- 범위 위배
INSERT INTO CHECK_TEST VALUES('JANG', 2003);
-- 얘도 초과
...
VALUES (001, 'USER1', 'PW001', '123456-1234567', '남', '01012341234','서울시 성북구', 'N');

INSERT INTO USER_TEST 
VALUES (002, 'USER2', 'PW002', '980324-1234567', '남', '01028344355','고양시 일산동구', 'N');

INSERT INTO USER_TEST 
VALUES (003, 'USER3', 'PW003', '950321-1234567', '남', '01087346328','고양시 일산서구', 'Y');

INSERT INTO USER_TEST 
VALUES (004, 'USER4', 'PW004', '970723-1234567', '남', '01019378423','서울시 관악구', 'N');

INSERT INTO USER_TEST 
VALUES (005, 'USER5', 'PW005', '001010-1234567', '남', '01098769876','서울시 강남구', 'N');
...
CREATE TABLE EMPLOYEE_COPY33 AS
...
ADD CONSTRAINT SAL_LEVEL_FK FOREIGN KEY(SAL_LEVEL)
...
ALTER TABLE EMPLOYEE 
ADD CONSTRAINT JOB_CODE_FK FOREIGN KEY(JOB_CODE)
...
ADD CONSTRAINT NATIONAL_CODE_FK FOREIGN KEY(NATIONAL_CODE)
